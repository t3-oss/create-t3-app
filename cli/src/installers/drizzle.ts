...
export const getDrizzleInstaller = (
  dialect: "sqlite" | "pscale"
): Installer => {
  switch (dialect) {
    case "sqlite":
      return sqliteDrizzleInstaller;
    case "pscale":
      return pscaleDrizzleInstaller;
    default:
      throw new Error(`Invalid dialect: ${dialect}`);
  }
};

const pscaleDrizzleInstaller: Installer = ({ projectDir, packages }) => {
...
const sqliteDrizzleInstaller: Installer = ({ projectDir, packages }) => {
  addPackageDependency({
    projectDir,
    dependencies: ["drizzle-kit", "dotenv", "@types/pg"],
    devMode: true,
  });
  addPackageDependency({
    projectDir,
