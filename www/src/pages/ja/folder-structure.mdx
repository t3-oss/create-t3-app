...
以下でパッケージを選択すると、その選択したパッケージで新たに足場を組んだアプリのフォルダ構造が表示されます。さらに下に進むと、各エントリーの説明があります。
...
また、マイグレーションファイルやシードスクリプトを格納する場所でもある（もし使用する場合）。
...
環境変数の検証や型定義に使用される - [環境変数](usage/env-variables)を参照。
...
`pages`フォルダには、Next.js アプリケーションのすべてのページが格納されます。`pages`のルートディレクトリにある `index.tsx` ファイルは、アプリケーションのホームページとなります。`\_app.tsx`ファイルは、アプリケーションを各種プロバイダでラップするために使用されます。詳しくは、[Next.js ドキュメント](https://nextjs.org/docs/basic-features/pages)を参照してください。
...
`trpc.ts`ファイルは、tRPC バックエンドのメイン設定ファイルです。この中で我々は

1. tRPC リクエストで使用するコンテキストを定義する。詳しくは[tRPC の使い方](usage/trpc#-serverapitrpcts)を参照してください。
2. プロシージャヘルパーをエクスポートします。詳しくは、[tRPC usage](usage/trpc#-serverapitrpcts) を参照してください。
...
`next-env.d.ts`ファイルは、Next.js の型が TypeScript コンパイラに拾われるようにするためのファイルです。**いつでも変更可能なので、削除や編集はしないでください。** 詳細は [Next.js Docs](https://nextjs.org/docs/basic-features/typescript#existing-projects) をご覧ください。
...
`next.config.mjs`ファイルは、Next.js を設定するためのものです。詳しくは[Next.js Docs](https://nextjs.org/docs/api-reference/next.config.js/introduction)を参照してください。注： .mjs という拡張子は、ESM のインポートを可能にするために使用されます。
...
`tsconfig.json`ファイルは、TypeScript を設定するために使用されます。Create T3 App とそのライブラリの TypeScript を最適に利用するために、`strict mode`のようないくつかの非デフォルトが有効になっています。詳細については、[TypeScript Docs](https://www.typescriptlang.org/docs/handbook/tsconfig-json.html) または [TypeScript Usage](usage/typescript) を参照してください。
