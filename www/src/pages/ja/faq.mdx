...
このプロジェクトで使用されているさまざまな技術に精通していない場合、それぞれのドキュメントを参照してください。それでも漠然としている場合は、私たちの[Discord](https://t3.gg/discord)に参加しヘルプしてください。
...
下記のリソースは T3 Stack のために存在する最高のものですが、コミュニティ（および[Theo](https://youtu.be/rzwaaWH0ksk?t=1436)）としては、まずはスタックを使い始め、それを使って開発をしながら学んでいくことをお奨めします。

もしあなたが Create T3 App を検討しているなら、すでにスタックの構成要素のいくつかを使っているかもしれません。そうなら、真っ先に飛び込んで、何かを作りながら他の構成要素を学んでいってみてはいかがでしょうか？

とはいえ、私たちは、この方法が誰にとってもうまくいくわけではないことも承知しています。もしあなたが、推奨される方法を試した上でもっとリソースが欲しいと感じたり、自分でやる自信がなかったり、スタックに圧倒されていると感じたら、以下にあるような Create T3 App の素晴らしいチュートリアルをチェックしてみてください：
...
## 自分のアプリケーションを国際化対応するのに苦労しています。何か参考になるものはないでしょうか？

i18n の組込みは非常に意見が分かれるトピックであり、実装の方法がたくさんあるため、`create-t3-app`にデフォルトで含めないことにしました。
...
[T3 原則 2](/ja/introduction#問題を解決する)の通り、私たちは最先端のものを愛していますが、安定性を重視しており、ルーター全体を移植するのは難しく、[血を流すのには適切なところではない](https://youtu.be/mnwUbtieOuI?t=1662)とみなしています。`/app` ディレクトリは [未来を垣間見せてくれる](https://youtu.be/rnsC-12PVlM?t=818)ものの、プロダクション投入の準備が整っているものではありません。その API はベータであり、互換性を損なう変更もあると予想されます。
