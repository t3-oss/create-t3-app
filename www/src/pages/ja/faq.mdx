...
- [Create T3 App でフルスタックケーションを作る](https://www.nexxel.dev/blog/ct3a-guestbook)
...
## 自分のアプリケーションに国際化対応するのに苦労しています。何か参考になるものはないでしょうか？

i18n の組込みは非常に意見が分かれるトピックであり、実装の方法がたくさんあるため、`create-t3-app`にデフォルトで含めることはしないことにしました。

しかし、実装に苦労があって参考プロジェクトを見たい場合は、[next-i18next](https://github.com/i18next/next-i18next)を使って T3 アプリケーションに i18n を追加する方法を紹介する[参考リポジトリ](https://github.com/juliusmarminge/t3-i18n)を用意しています。
...
[T3-公理#2](/ja/introduction#問題を解決する)の通り、私たちは最先端のものを愛していますが、安定性を重視しており、ルーター全体を移植するのは難しく、[血を流すのには適切なところではない](https://youtu.be/mnwUbtieOuI?t=1662)とみなしています。その API はベータ版であり、変更されると予想されます。
