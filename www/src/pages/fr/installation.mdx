...
| `--drizzle`               | Inclure Drizzle dans le projet                  |
| `--nextAuth`              | Inclure NextAuth.js dans le projet              |
| `--tailwind`              | Inclure Tailwind CSS dans le projet             |
| `--dbProvider [provider]` | Inclure une base de données dans le projet      |
| `--appRouter`             | Utiliser l'App Router de Next.js dans le projet |
...
Vous n'avez pas besoin de vous désinscrire explicitement des packages que vous
ne voulez pas.Cependant, si vous préférez être explicite, vous pouvez passer
`false`, par exemple `--nextAuth false`.
...
Ce qui suit configurera une application T3 avec NextAuth.js, Tailwind CSS, Drizzle et PostgreSQL.
