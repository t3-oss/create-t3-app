...
يحتوي مجلد `common` على الوظائف (Functions) التي تكثر استخدامها.
...
يستخدم ملف `get-server-auth-session.ts` لجلب الجلسة (Session) الـ NextAuth.js من جانب السيرفر. اقرأ [NextAuth.js usage](usage/next-auth#usage-with-trpc) لمزيد من المعلومات.
...
يُستخدم ملف `context.ts` لإنشاء محتوى في كل طلب tRPC، اقرأ [tRPC usage](usage/trpc#-servertrpccontextts) لمزيد من المعلومات.
...
يستخدم `trpc.ts` لنشر الإجرات المساعدة للتطبيق، اقرأ [tRPC usage](usage/trpc#-servertrpctrpcts) لمزيد من المعلومات.
...
يُستخدم ملف `_app.ts` لدمج tRPC routers إلى router واحد، وكذلك مع الـ types المحددة، اقرأ المزيد هنا [tRPC usage](usage/trpc#-servertrpcrouterts).
...
يكون ملف `example.ts` مثال عن استخدام `publicProcedure` لإنشاء tRPC route عام.
