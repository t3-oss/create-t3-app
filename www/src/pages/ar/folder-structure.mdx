...
مجلد `pages` يحتوي علي كل الصفحات التي يحتوي عليها تطبيق Next.js، يعمل ملف `index.tsx` في مستند root (الأولي) من `/pages` كالصفحة الأساسية للتطبيق،
...
يحتوي مجلد `api` علي كل الـ Api Endpoints التي يحتويها تطبيقك، يحتوي `example.ts` (مع Prisma) على مثال عن طريق (route) الذي يستخدم [Next.js API route](https://nextjs.org/docs/api-routes/introduction) مع Prisma. يحتوي ملف `restricted.ts` علي مثال لـ Endpoint مع (Next-Auth)، إقرأ [Next.js API route](https://nextjs.org/docs/api-routes/introduction) و [NextAuth.js](https://next-auth.js.org/).
