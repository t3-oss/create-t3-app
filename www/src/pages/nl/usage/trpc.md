---
title: tRPC
description: gebruik van tRPC
layout: ../../../layouts/docs.astro
lang: nl
---

tRPC laat ons end-to-end typesafe APIs shcrijven zonder codegeneratie of runtime bloat. Het maakt gebruik van de geweldige infereren van TypeScript om de type definities van uw API-router te inferreren en laat je je API procedures op te roeepen vanuit je front-end op te roepen met volledige typesafety en autocompletie. Bij het gebruik van tRPC voelt je front- en backend dichterbij dan ooit, wet leidt tot een uitstekende ontwikkelaars beleving.
...
<blockquote className="w-full relative border-l-4 italic bg-t3-purple-200 dark:text-t3-purple-50 text-zinc-900 dark:bg-t3-purple-300/20 p-2 rounded-md text-sm my-3 border-neutral-500 quote">
  ...
  <cite className="flex items-center justify-end pr-4 pb-2">
    ...
    <div className="flex flex-col items-start not-italic">
      <span className=" text-sm font-semibold">Alex - maker van tRPC</span>
      <a
...
tRPC vereist behoorlijk veel boilerplate die `create-t3-app` voor u instelt. Laten we eens kijken naar de gegenereerde bestanden:

### 📄 `pages/api/trpc/[trpc].ts`

Dit is het begin van je API en maakt de tRPC router zichtbaar. Normaal gesproken zul je dit bestant niet vaak aanpassen, maar als je bijvoorbeeld CORS-middleware of iets dergelijks wilt inschakelen, is het nuttig om te weten dat de geëxporteerde `createNextApiHandler` een [Next.js API handler](https://nextjs.org/docs/api-routes/introduction) is die een [request](https://developer.mozilla.org/en-US/docs/Web/API/Request) en [response](https://developer.mozilla.org/en-US/docs/Web/API/Response)  object aanneemt. Dit betekent dat je de `createNextApiHandler` kan wrappen met iedere middelware die je wilt. Kijk [hier](#enabling-cors) voor een voorbeeld met CORS.

### 📄 `server/trpc/context.ts`

Dit bestand is waar je de context definieert die aan uw tRPC-procedures wordt doorgegeven. Context is de data waar al je tRPC-proceduren toegang tot hebben, en is een geweldige plek om dingen te plaatsen zoals databaseverbindingen, authenticatie-informatie, enz. In `create-t3-app` gebruiken we twee functies om het mogelijk te maken om een deel van de context te gebruiken wanneer we geen toegang hebben tot het aanvraagobject.

- `createContextInner`: Dit is waar je de context definieert die niet afhankelijk is voor de aanvraag, bijv. je databaseverbinding. je kunt deze functie gebruiken voor [integratietests](#sample-integration-test) of [ssg-helpers](https://trpc.io/docs/v10/ssg-helpers) waar je geen aanvraagobject hebt.

- `createContext`: Dit is waar je de context definieert die afhankelijk is voor de aanvraag, bijv. de sessie van de gebruiker. Je vraagt de sessie aan met het `opts.req` object en geeft vervolgens de sessie door aan de `createContextInner` functie om de eindcontext te maken.
...
This is where you define the routes a nd procedures of your API. By convention, you [create separate routers](https://trpc.io/docs/v10/router) for related procedures, then [merge](https://trpc.io/docs/v10/merging-routers) all of them into a single app router in `server/trpc/router/_app.ts`.
