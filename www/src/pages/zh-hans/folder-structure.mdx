...
1. 定义在 tRPC 请求中使用的上下文。 参看 [tRPC 用法](usage/trpc#-servertrpccontextts) 来了解更多。
2. 导出 procedure。 参看 [tRPC 用法](usage/trpc#-servertrpctrpcts) 来了解更多。
...
文件 `root.ts` 被用于合并 tRPC 子路由并将它们导出为一个单一的路由，同时也导出了路由的类型定义。参看 [tRPC 用法](usage/trpc#-servertrpcrouterts) 来了解更多。
